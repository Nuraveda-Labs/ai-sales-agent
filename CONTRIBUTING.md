...
Thanks for thinking about contributing. This agent is one of six in the open-source [Mesh Pilot](https://meshpilot.app) marketing stack, and we welcome PRs that fit its scope: outbound B2B sales with a human-in-the-loop approval gate before send.

## Ways to contribute

- **Bug reports** — open an issue on either mirror (GitLab is primary; Codeberg syncs nightly). Include the integration involved (Google Places / LLM / Resend / Discord), the action that failed, and any redacted log output.
